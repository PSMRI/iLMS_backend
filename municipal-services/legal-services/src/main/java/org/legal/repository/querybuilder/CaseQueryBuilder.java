...
//                preparedStmtList.add('%' + criteria.getUuid() + '%');
                preparedStmtList.add( criteria.getUuid() );
//                builder.append("  = ?").append(createQuery(Collections.singleton(uuid))).append(")");
//                addToPreparedStatement(preparedStmtList, Collections.singleton(uuid));
            }
        } catch (NullPointerException e) {
            preparedStmtList.add("");
        }
