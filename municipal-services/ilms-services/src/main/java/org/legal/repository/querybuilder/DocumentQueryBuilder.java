...
import org.legal.web.model.DocumentSearchCriteria;
...
public class DocumentQueryBuilder {
    private static final String Query = "select count(*) OVER() AS document_full_count, id, case_id, document_type, file_store_id, status, remarks,document_id, createdby, createdtime, lastmodifiedby, lastmodifiedtime from eg_lg_document ";
    ...
    public String getDocumentSearchQuery(DocumentSearchCriteria criteria, List<Object> preparedStmtList) {
        ...
        if (criteria.getDocumentType() != null) {
            if (criteria.getDocumentType().split("\\.").length == 1) {
                ...
                builder.append(" eg_lg_document.document_type like ?");
                preparedStmtList.add('%' + criteria.getDocumentType() + '%');
            } else {
                addClauseIfRequired(preparedStmtList, builder);
                builder.append(" eg_lg_document.document_type = ?");
                preparedStmtList.add('%' + criteria.getDocumentType() + '%');
            }
        }
        ...
        try {
            if (!CollectionUtils.isEmpty(caseId)) {
                addClauseIfRequired(preparedStmtList, builder);
                builder.append(" eg_lg_document.case_id IN (").append(createQuery(caseId)).append(")");
        ...
        try {
            if (!CollectionUtils.isEmpty(fileStoreId)) {
                addClauseIfRequired(preparedStmtList, builder);
                builder.append(" eg_lg_document.file_store_id IN (").append(createQuery(fileStoreId)).append(")");
        ...
        try {
            if (!CollectionUtils.isEmpty(id)) {
                addClauseIfRequired(preparedStmtList, builder);
                builder.append(" eg_lg_document.id IN (").append(createQuery(id)).append(")");
    ...
    private void addOrderByClause(StringBuilder builder, DocumentSearchCriteria criteria) {
        if (criteria.getSortBy() == DocumentSearchCriteria.SortBy.caseId) {
            builder.append(" ORDER BY eg_lg_document.case_id ");
        } else if (criteria.getSortBy() == DocumentSearchCriteria.SortBy.documentType) {
            builder.append(" ORDER BY eg_lg_case.document_type ");
        }
