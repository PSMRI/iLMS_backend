...
public class JudgementValidator {
    ...
    public void createValidator(JudgementRequest request) {
        ...
        if (!StringUtils.isNotBlank(request.getJudgement().getOrderType())) {
            throw new CustomException(LegalErrorConstants.INVALID_TYPE_ERROR, "orderType is mandatory");
        }
        if (!StringUtils.isNotBlank(request.getJudgement().getOrderDate().toString())) {
            throw new CustomException(LegalErrorConstants.INVALID_TYPE_ERROR, "orderDate is mandatory");
        }
        if (StringUtils.isNotBlank(request.getJudgement().getDecisionStatus())) {
            throw new CustomException(LegalErrorConstants.INVALID_TYPE_ERROR, "decisionStatus is not allowed while creating judgement");
        }
        if (!StringUtils.isNotBlank(request.getJudgement().getOrderNoOverride())) {
            throw new CustomException(LegalErrorConstants.INVALID_TYPE_ERROR, "orderNoOverride is mandatory");
        }
        if (!StringUtils.isNotBlank(request.getJudgement().getRevisedComplainceReason())) {
            throw new CustomException(LegalErrorConstants.INVALID_TYPE_ERROR, "revisedComplianceReason is mandatory");
        }
        if (StringUtils.isNotBlank(request.getJudgement().getComplianceStatus())) {
            throw new CustomException(LegalErrorConstants.INVALID_TYPE_ERROR, "complianceStatus is not allowed while creating judgement");
        }
        if (!StringUtils.isNotBlank(request.getJudgement().getRemarks())) {
            throw new CustomException(LegalErrorConstants.INVALID_TYPE_ERROR, "remarks is mandatory");
        }
