...
public class WorkflowService {
	 ...
	 * @param request
	 */
	public State updateWorkflow(ILMSCaseRequest request, CreationReason creationReasonForWorkflow) {
		...
		if (state.getApplicationStatus().equalsIgnoreCase(configs.getWfStatusActive()) && ilmsCase.getId() == null) {
			
			String pId = utils.getIdList(request.getRequestInfo(), ilmsCase.getTenantId(), configs.getCaseIdgenName(), configs.getCaseIdgenFormat(), 1).get(0);
			request.getIlmsCase().setId(pId);
		}

		request.getIlmsCase().getWorkflow().setState(state);
		return state;
	}
