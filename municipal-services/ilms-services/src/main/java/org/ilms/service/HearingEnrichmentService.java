...
public class HearingEnrichmentService {
   ...
   private ILMSCaseRepository ilmsCaseRepository;
    ...
    private void setIdgenIds(HearingRequest request) {
        ...
        List<String> applicationNumbers = getIdList(requestInfo, tenantId, config.getHearingIdgenName(),
                config.getHearingIdgenFormat(), 1);
        ListIterator<String> itr = applicationNumbers.listIterator();
        List<String> courtId = getIdList(requestInfo, tenantId, config.getCourtIdGenName(),
                config.getCourtIdGenFormat(), 1);
