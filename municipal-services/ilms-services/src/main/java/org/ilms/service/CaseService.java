...
public class CaseService {
    ...
    public CaseSearchResponse ilmsCaseSearch(CaseSearchCriteria criteria, RequestInfo requestInfo, ProcessInstanceSearchCriteria processInstanceSearchCriteria) {
        ...
        CaseSearchResponse finalResult = new CaseSearchResponse();
    ...
    public CaseDetailsResponse caseDetailsSearch(CaseSearchCriteria criteria, RequestInfo requestInfo) {
        ...
        OrderSearchCriteria judgementSearchCriteria = OrderSearchCriteria.builder().caseId(Collections.singletonList(
                caseResponse.getCaseList().get(0).getId())).build();
        orderResponse = orderRepository.getOrderData(judgementSearchCriteria);
        caseResponse.getCaseList().forEach(caseObj -> {
        ...
        orderResponse.getOrderList().forEach(judgement -> {
    ...
    public Case create(CaseRequest caseRequest) {
        if (Objects.nonNull(caseRequest.getCaseObj().getCaseHierarchy())) {
            if (caseRequest.getCaseObj().getCaseHierarchy().equals(CaseHierarchy.INDEPENDENT)) {
                ...
                CaseSearchResponse response = caseRepository.getILMSCaseData(criteria);
        ...
        if (Objects.nonNull(caseRequest.getCaseObj().getPartyList())) {
            for (Party party : caseRequest.getCaseObj().getPartyList()) {
            ...
            if (!caseResponse.getCaseList().isEmpty()) {
                ...
                if (Objects.nonNull(caseRequest.getCaseObj().getCaseHierarchy())) {
                    if (caseRequest.getCaseObj().getCaseHierarchy().equals(CaseHierarchy.INDEPENDENT)) {
                        ...
                        CaseSearchResponse response = caseRepository.getILMSCaseData(criteria1);
                        if (response.getCaseList().size() != 1) {
                            throw new CustomException(ILMSErrorConstants.PARENT_CASE_NOT_FOUND, "Parent Case does not exist");
                        }
                    }
...
//
//                Paragraph caseFlag = new Paragraph(new Chunk(ilmsCase.getCaseFlag()));
...
//                cell5.setPaddingTop(-1f);
//
//                Paragraph hid = new Paragraph(new Chunk(hearing.getCourt().getHearingId()));
//                cell6.setMinimumHeight(12f);
...
//                cell7.setPaddingTop(-1f);
//
//                Paragraph courtno = new Paragraph(new Chunk(hearing.getCourt().getCourtNumber()));
//                cell8.setMinimumHeight(12f);
...
//                cell9.setPaddingTop(-1f);
//
//                Paragraph courtname = new Paragraph(new Chunk(hearing.getCourt().getCourtName()));
//                cell10.setMinimumHeight(12f);
